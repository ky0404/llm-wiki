...
## 描述
Obsidian 是一款基于 Markdown 的知识管理和笔记应用，使用本地文件存储，支持双向链接、图形视图和丰富的插件生态系统。在 [[concepts/llm-wiki]] 系统中，Obsidian 被用作"IDE"，而 LLM 是"程序员"。
...
2. **双向链接**：支持 [[concepts/wikilinks]] 格式，实现页面间的交叉引用
...
## 与 LLM Wiki 的集成
在 [[concepts/llm-wiki]] 架构中：
1. **原始源** → 存储在 `raw/` 目录中
2. **Wiki** → 存储在 `wiki/` 目录中，Obsidian 作为查看器
3. **模式** → 存储在 `wiki/AGENTS.md` 中，指导 LLM 操作

## References
- [[concepts/llm-wiki]] - 使用 Obsidian 作为 IDE
- [[entities/marp]] - 推荐的幻灯片插件
- [[entities/dataview]] - 推荐的数据查询插件
- [[concepts/wikilinks]] - Obsidian 支持的双向链接格式