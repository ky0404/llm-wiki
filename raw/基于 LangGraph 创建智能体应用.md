...
source: "https://www.aneasystone.com/archives/2024/10/create-agents-with-langgraph.html"
author:
published:
created: 2026-04-30
description: "早在年初的时候，LangChain 发布了 v0.1.0 稳定版本，版本公告里通过大量的篇幅对功能特性做了全面的介绍，最后，在公告的结尾，提到了一个不那么显眼的库，那就是 LangGraph。尽..."
tags:
  - "clippings"
---
...
在之前的 LangChain 版本中，我们可以通过 `AgentExecutor` 实现智能体，在 [大模型应用开发框架 LangChain 学习笔记（二）](https://www.aneasystone.com/archives/2023/09/llm-application-frameworks-langchain-2.html) 中，我们曾经学习过 `AgentExecutor` 的用法，实现了包括 Zero-shot ReAct Agent、Conversational ReAct Agent、ReAct DocStore Agent、Self-Ask Agent、OpenAI Functions Agent 和 Plan and execute Agent 这些不同类型的智能体。但是这种方式过于黑盒，所有的决策过程都隐藏在 `AgentExecutor` 的背后，缺乏更精细的控制能力，在构建复杂智能体的时候非常受限。
...
![basic-chatbot.jpg](https://www.aneasystone.com/usr/uploads/2024/10/1656838660.jpg "basic-chatbot.jpg")
...
![agent-overview.png](https://www.aneasystone.com/usr/uploads/2024/10/2388301544.png "agent-overview.png")
...
这里使用了 LangChain 的 `@tool` 注解将一个方法定义成工具，并使用了 `pydantic` 对工具的参数做一些说明，在 [这篇博客](https://www.aneasystone.com/archives/2023/09/llm-application-frameworks-langchain-2.html) 中我还介绍了一些其他定义工具的方法，也可以使用。
...
![tools-chatbot.jpg](https://www.aneasystone.com/usr/uploads/2024/10/1054596440.jpg "tools-chatbot.jpg")
...
![tool-calling-flow.png](https://www.aneasystone.com/usr/uploads/2024/10/1458089590.png "tool-calling-flow.png")

[OpenAI 官方文档](https://platform.openai.com/docs/guides/function-calling) 中有一张更详细的流程图：

![function-calling-diagram.png](https://www.aneasystone.com/usr/uploads/2024/10/3578158606.png "function-calling-diagram.png")
...
![memory-db.png](https://www.aneasystone.com/usr/uploads/2024/10/1560061424.png "memory-db.png")
...
![langsmith-runs.png](https://www.aneasystone.com/usr/uploads/2024/10/3080471878.png "langsmith-runs.png")

`Runs` 列表表示智能体每次的运行记录，也可以切换到 `Threads` 列表查看所有的会话线程：

![langsmith-threads.png](https://www.aneasystone.com/usr/uploads/2024/10/792157130.png "langsmith-threads.png")

点击进入记录详情，可以很直观地看到 LangGraph 的调用顺序，每一步的耗时和令牌数一目了然：

![langsmith-thread-details.png](https://www.aneasystone.com/usr/uploads/2024/10/2727731324.png "langsmith-thread-details.png")
...
![langsmith-monitor.png](https://www.aneasystone.com/usr/uploads/2024/10/3306014094.png "langsmith-monitor.png")
...
**检索增强生成（Retrieval-Augmented Generation，简称 RAG）** 通过引入外部信息源实现知识问答，解决大模型缺乏领域知识、无法获取实时信息以及生成虚假内容等问题。我们在 [这篇博客](https://www.aneasystone.com/archives/2024/06/advanced-rag-notes.html) 中学习了不少高级 RAG 技巧，通过 LangGraph 可以将智能体和 RAG 相结合，实现更好的问答效果。
