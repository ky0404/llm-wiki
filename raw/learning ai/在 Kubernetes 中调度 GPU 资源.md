---
title: "在 Kubernetes 中调度 GPU 资源"
source: "https://www.aneasystone.com/archives/2023/12/scheduling-gpus-in-kubernetes.html"
author:
published:
created: 2026-04-30
description: "在人工智能越来越普及的今天，GPU 也变得越来越常见，无论是传统的机器学习和深度学习，还是现在火热的大语言模型和文生图模型，GPU 都是绕不开的话题。最近在工作中遇到一个需求，需要在 Kuber..."
tags:
  - "clippings"
---
...
学习 GPU 有一定的门槛，不仅是因为好点的显卡都价格不菲，而且使用它还要搭配有相应的硬件环境，虽然笔记本也可以通过显卡扩展坞来使用，但是性能有一定的损失。对于有条件的同学，网上有很多关于如何搭建自己的深度学习工作站的教程可供参考，对此我也没有什么经验，此处略过；对于没有条件的同学，网上也有很多白嫖 GPU 的攻略，我在 [使用 Google Colab 体验 AI 绘画](https://www.aneasystone.com/archives/2023/04/ai-painting-with-google-colab.html) 这篇博客中也介绍了如何在 Google Colab 中免费使用 GPU 的方法；不过这些环境一般都是做机器学习相关的实验，如果想在上面做一些更底层的实验，比如安装 Docker，部署 Kubernetes 集群等，就不太合适了。
...
![aliyun-ecs.png](https://www.aneasystone.com/usr/uploads/2023/12/1431349830.png "aliyun-ecs.png")

于是便抱着试一试的态度下了一单，然后开始了下面的实验。但是刚开始就遇到了问题，安装 NVIDIA 驱动的时候一直报 `Unable to load the kernel module 'nvidia.ko'` 这样的错误:

![nvidia-driver-error.png](https://www.aneasystone.com/usr/uploads/2023/12/3802580825.png "nvidia-driver-error.png")
...
![nvidia-driver-download.png](https://www.aneasystone.com/usr/uploads/2023/12/2246859079.png "nvidia-driver-download.png")
...
![cuda-download.png](https://www.aneasystone.com/usr/uploads/2023/12/272780545.png "cuda-download.png")

找到 `12.2` 版本进入下载页面：

![cuda-download-2.png](https://www.aneasystone.com/usr/uploads/2023/12/3880010892.png "cuda-download-2.png")
...
GPU 环境准备好之后，接下来我们先试着在 Docker 容器中使用它。由于是新买的系统，并没有 Docker 环境，所以我们要先安装 Docker，可以参考我之前写的 [在 VirtualBox 上安装 Docker 服务](https://www.aneasystone.com/archives/2022/02/install-docker.html) 这篇博客。
...
终于到了这篇博客的主题，接下来我们实践一下如何在 Kubernetes 集群中调度 GPU 资源。和 Docker 一样，新买的服务器上也没有 Kubernetes 环境，我们需要先安装 Kubernetes，可以参考我之前写的 [Kubernetes 安装小记](https://www.aneasystone.com/archives/2022/05/install-kubernetes.html) 这篇博客。
