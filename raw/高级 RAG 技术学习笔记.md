---
title: "高级 RAG 技术学习笔记"
source: "https://www.aneasystone.com/archives/2024/06/advanced-rag-notes.html"
author:
published: 2024-07-23
created: 2026-04-30
description: "随着大模型技术的发展，基于大模型开发的应用也越来越多，比如类似 ChatGPT 的对话服务，将搜索引擎与大模型相结合的问答服务，等等。但在这些应用中，我们也面临着大量的问题，包括缺乏领域知识、无..."
tags:
  - "clippings"
---
...
我们在之前的笔记中已经学习过不少和 RAG 相关的内容，比如在 [使用 Embedding 技术打造本地知识库助手](https://www.aneasystone.com/archives/2023/07/doc-qa-using-embedding.html) 这篇笔记中，我们学习了如何打造一个针对本地文档的问答系统，在 [基于结构化数据的文档问答](https://www.aneasystone.com/archives/2023/11/structured-data-qa.html) 这篇笔记中，我们继续探索了如何针对结构化的数据进行问答。不过这些内容都比较简单，只是对 RAG 原理的入门级讲解，本篇博客将对 RAG 的高级技巧进行深入学习，并结合 LangChain 和 LlamaIndex 对各个技巧一一进行实战。

## RAG 概述

RAG 的本质是搜索 + LLM 提示（Search + LLM prompting），根据用户的问题，通过一定的搜索算法找到相关的信息，将其注入到大模型的提示中，然后令大模型基于上下文来回答用户的问题。其工作流程如下图所示：

![rag-overview.png](https://www.aneasystone.com/usr/uploads/2024/06/1670465452.png "rag-overview.png")
...
![rag-paradigms.png](https://www.aneasystone.com/usr/uploads/2024/06/680934063.png "rag-paradigms.png")
...
![7-failure-points.png](https://www.aneasystone.com/usr/uploads/2024/06/4042333692.png "7-failure-points.png")
...
![12-pain-points.png](https://www.aneasystone.com/usr/uploads/2024/06/4176403959.png "12-pain-points.png")
...
![basic-rag.png](https://www.aneasystone.com/usr/uploads/2024/06/3806730310.png "basic-rag.png")
...
![advanced-rag.jpeg](https://www.aneasystone.com/usr/uploads/2024/06/4280944004.jpeg "advanced-rag.jpeg")
...
![rag-fusion.png](https://www.aneasystone.com/usr/uploads/2024/06/746192718.png "rag-fusion.png")
...
**后退提示（Step-Back Prompting）** 是 Google DeepMind 团队在论文 [Take a Step Back: Evoking Reasoning via Abstraction in Large Language Models](https://arxiv.org/abs/2310.06117) 中提出的一种新的提示技术，我在 [之前的笔记中](https://www.aneasystone.com/archives/2023/08/llm-application-frameworks-langchain.html) 已经介绍过后退提示的基本原理。总的来说，它基于用户的原始问题生成一个后退问题，后退问题相比原始问题具有更高级别的概念或原则，从而提高解决复杂问题的效果，例如一个关于物理学的问题可以后退为一个关于该问题背后的物理原理的问题，然后对原始问题和后退问题进行检索。
...
![query-construction.png](https://www.aneasystone.com/usr/uploads/2024/06/4278736256.png "query-construction.png")
...
为了大模型能生成准确的 SQL，我们必须将数据库表的信息完整的送入大模型的上下文中，如果数据库表或列过多，就会超出大模型的 token 限制。这时，我们必须找到方法，动态地仅插入最相关的信息到提示中。我们可以使用 LangChain 内置的 [create\_extraction\_chain\_pydantic](https://python.langchain.com/v0.1/docs/use_cases/sql/large_db/) 链来实现这点，它通过 OpenAI 的 funtion call 功能动态地挑出和用户问题最相关的表，然后再基于这些表生成 SQL 语句；LlamaIndex 的 [SQLTableRetrieverQueryEngine](https://docs.llamaindex.ai/en/stable/examples/index_structs/struct_indices/SQLIndexDemo/#part-2-query-time-retrieval-of-tables-for-text-to-sql) 也实现了同样的功能，它通过为每个表生成一个嵌入向量来实现这一点。
...
和 Text-to-SQL 一样，元数据过滤也面临着大模型生成的过滤条件可能和库中的元数据无法完全匹配的问题，比如：库中的字段是大写，而用户的输入是小写，库中的字段是全称，而用户的输入是简称，这时我们也可以借鉴 Text-to-SQL 中的优化手段，比如自定义 Prompt 或 根据用户输入动态选择样本， [这里](https://docs.llamaindex.ai/en/stable/examples/vector_stores/pinecone_auto_retriever/#part-2-extending-auto-retrieval-with-dynamic-metadata-retrieval) 是 LlamaIndex 的示例。此外，LlamaIndex 官网还有一篇使用元数据过滤实现 [多文档检索（或者叫结构化分层检索）](#)) 的示例。
...
除了上面所说的 Markdown、HTML、JSON 等结构化文本，还有很多代码格式的文件，不同的编程语言拥有不同的关键字和语法，分块方式也略有区别。LangChain 为每种编程语言预定义了对应的分隔符，我们可以直接使用 [RecursiveCharacterTextSplitter.from\_language()](https://python.langchain.com/v0.1/docs/modules/data_connection/document_transformers/code_splitter/) 为特定语言创建文本分割器：
...
![parent-retrieve.jpeg](https://www.aneasystone.com/usr/uploads/2024/06/3786848725.jpeg "parent-retrieve.jpeg")
...
![parent-retrieve-data.png](https://www.aneasystone.com/usr/uploads/2024/06/2093216291.png "parent-retrieve-data.png")
...
![hierarchical-retrieval.png](https://www.aneasystone.com/usr/uploads/2024/06/355465261.png "hierarchical-retrieval.png")
...
![rrf-score.png](https://www.aneasystone.com/usr/uploads/2024/06/1253633240.png "rrf-score.png")
...
![long-context.png](https://www.aneasystone.com/usr/uploads/2024/06/2672260541.png "long-context.png")
...
![sentence-window.png](https://www.aneasystone.com/usr/uploads/2024/06/1892057486.png "sentence-window.png")
...
![prev-next.png](https://www.aneasystone.com/usr/uploads/2024/06/3923438944.png "prev-next.png")
