---
title: "提示工程学习笔记（二）"
source: "https://www.aneasystone.com/archives/2024/02/prompt-engineering-notes-2.html"
author:
published:
created: 2026-04-30
description: "在 上一篇笔记 中，我们学习了很多提示工程相关的技术，比如思维链（CoT）和最小到最多提示（Least-to-Most Prompting）等，显著改善了大模型的推理能力。尽管如此，我们常常还是..."
tags:
  - "clippings"
---
在 [上一篇笔记](https://www.aneasystone.com/archives/2024/01/prompt-engineering-notes.html) 中，我们学习了很多提示工程相关的技术，比如思维链（CoT）和最小到最多提示（Least-to-Most Prompting）等，显著改善了大模型的推理能力。尽管如此，我们常常还是会看到这样的现象：大模型可以准确地生成解决问题的逻辑步骤，但最终结果仍然不正确，通常这个结果是由于非常简单的错误引起的，比如数值计算错误、无法理解私有知识等。因此研究人员又提出很多想法希望对语言模型进行增强，最常见的思路有：检索增强、编程增强和工具增强，这样的语言模型被称为 [增强语言模型（Augmented Language Models）](https://arxiv.org/abs/2302.07842) 。
...
![webgpt.png](https://www.aneasystone.com/usr/uploads/2024/02/1766834489.png "webgpt.png")
...
![internet-agumented-llm.png](https://www.aneasystone.com/usr/uploads/2024/02/1891890776.png "internet-agumented-llm.png")
...
![pal.png](https://www.aneasystone.com/usr/uploads/2024/02/1882546681.png "pal.png")
...
![pot.png](https://www.aneasystone.com/usr/uploads/2024/02/701135599.png "pot.png")
...
![pot-few-zero.png](https://www.aneasystone.com/usr/uploads/2024/02/1710786199.png "pot-few-zero.png")

## 工具增强

检索增强扩展了模型获取信息的能力，编程增强扩展了模型解决问题的能力，如果抽象来看，他们实际上都是外部工具的调用，让模型负责推理，推理之外的事通过调用外部工具来实现。在 [大模型应用开发框架 LangChain 学习笔记（二）](https://www.aneasystone.com/archives/2023/09/llm-application-frameworks-langchain-2.html) 中，我们学习了 OpenAI 的插件机制和 Function Calling 功能，这些其实都是通过外部工具实现的。
...
![talm-vs-lm.png](https://www.aneasystone.com/usr/uploads/2024/02/1096481292.png "talm-vs-lm.png")
...
![talm-examples.png](https://www.aneasystone.com/usr/uploads/2024/02/257643002.png "talm-examples.png")
...
![toolformer.png](https://www.aneasystone.com/usr/uploads/2024/02/2100402643.png "toolformer.png")
...
![toolformer-train.png](https://www.aneasystone.com/usr/uploads/2024/02/1116205947.png "toolformer-train.png")
...
Output:
```
1. **Sample API Calls** ：将每一个 API 调用表示为一个二元组（API 的名称和相应的输入），对于同一个位置 `i` ，我们进行多次采样，生成不同的 API 调用 `ci1` 、 `ci2` 等；
2. **Excute API Calls** ：执行上面生成的每个 API 调用得到结果 `ri1` 、 `ri2` 等；
...
![art.png](https://www.aneasystone.com/usr/uploads/2024/02/3717104550.png "art.png")
...
![agent-overview.png](https://www.aneasystone.com/usr/uploads/2024/02/3673698422.png "agent-overview.png")
...
![mrkl-error-examples.png](https://www.aneasystone.com/usr/uploads/2024/02/2120130713.png "mrkl-error-examples.png")

尽管存在这些缺点，但 AI21 Labs 仍然认为，大型语言模型是未来人工智能系统的重要支柱。为解决这些问题，他们提出 MRKL 解决方案，概要设计如下：

![mrkl-system.png](https://www.aneasystone.com/usr/uploads/2024/02/4175429126.png "mrkl-system.png")
...
![react.png](https://www.aneasystone.com/usr/uploads/2024/02/3149710048.png "react.png")

下图是 ReAct 和其他提示技术的一个对比：

![react-hotspot-qa.png](https://www.aneasystone.com/usr/uploads/2024/02/1454623592.png "react-hotspot-qa.png")
...
ReAct 的实现代码在 [GitHub](https://github.com/ysymyth/ReAct) 上开源了，有兴趣同学的可以尝试下。另外，LangChain 基于 ReAct 的思想实现了 [Zero-shot ReAct Agent](https://python.langchain.com/docs/modules/agents/agent_types/react) ，关于它的使用方法可以参考我之前写的 [大模型应用开发框架 LangChain 学习笔记](https://www.aneasystone.com/archives/2023/09/llm-application-frameworks-langchain-2.html) 。
...
![self-ask.png](https://www.aneasystone.com/usr/uploads/2024/02/1644104300.png "self-ask.png")
...
![plan-and-solve.png](https://www.aneasystone.com/usr/uploads/2024/02/687458413.png "plan-and-solve.png")
...
所以单独使用 Plan-and-Solve Prompting 在智能体中作用并不大，一般使用这种思想来将用户的问题拆解成子任务，然后每个子任务再使用传统的 Action Agent 进行处理。在 [大模型应用开发框架 LangChain 学习笔记（二）](https://www.aneasystone.com/archives/2023/09/llm-application-frameworks-langchain-2.html) 中，我们学习了 [Plan and execute Agent](https://blog.langchain.dev/plan-and-execute-agents/) 的概念和用法，它的基本思想就来自于此。

![plan-and-execute.png](https://www.aneasystone.com/usr/uploads/2024/02/674439783.png "plan-and-execute.png")
...
![hugginggpt.png](https://www.aneasystone.com/usr/uploads/2024/02/2284242305.png "hugginggpt.png")
...
![hugginggpt-overview.png](https://www.aneasystone.com/usr/uploads/2024/02/461884303.png "hugginggpt-overview.png")
